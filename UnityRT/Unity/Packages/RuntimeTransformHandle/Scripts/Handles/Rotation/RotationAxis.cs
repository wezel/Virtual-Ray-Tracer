...
namespace RuntimeHandle
{
    ...
    public class RotationAxis : HandleBase
    {
        ...
        public RotationAxis Initialize(RuntimeTransformHandle p_runtimeHandle, Vector3 p_axis, Vector3 p_perp, Color p_color)
        {
            ...
            mf.mesh = MeshUtils.CreateTorus(2f, .04f, 32, 6);
        ...
        public override void Interact(Vector3 p_previousPosition)
        {
            ...
            if (_parentTransformHandle.space == HandleSpace.LOCAL)
            {
                ...
                _arcMesh = MeshUtils.CreateArc(transform.position, _hitPoint, rotatedAxis, 2 * transform.lossyScale.x, -snappedDelta,
            ...
            else
            {
                ...
                _arcMesh = MeshUtils.CreateArc(transform.position, _hitPoint, _axis, 2 * transform.lossyScale.x, -snappedDelta,
                    Mathf.Abs(Mathf.CeilToInt(snappedDelta * 180 / Mathf.PI)) + 1);
                DrawArc();
        ...
        void DrawArc()
        {
            ...
            Graphics.DrawMesh(_arcMesh, Matrix4x4.identity /*Matrix4x4.Scale(transform.lossyScale)*/ /* Matrix4x4.Translate(transform.position)*/, _arcMaterial, 0);
